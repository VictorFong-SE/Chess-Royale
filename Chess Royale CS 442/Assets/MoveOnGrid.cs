...
public class MoveOnGrid : MonoBehaviour
{
    ...
    private void makeMove()
    {
        ...
        if(playerIndex == 0 && displayedMoves == false)
        {
            ...
            for (int i = 0; i < validSqs.Count; i++)
            {
                ...
                Color YELLOW = new Color(193f, 150f, 0f, .75f);
        ...
        if ((playerIndex == 1 || playerIndex == 2 || playerIndex == 3) && animating == false)
        {
            ...
            int randx = Random.Range(0, GoBetween.boardSize);
            int randy = Random.Range(0, GoBetween.boardSize);
    ...
    private void downGradePlayerPiece()
    {
        bool isPlayer = false;
        string currPiece = player.GetComponent<Piece>().GetName();
        if (script.grid.GetPlayer(0) == player) {
            isPlayer = true;
        }
        DestroyImmediate(player.GetComponent<Piece>());
        switch (currPiece)
        {
            case "king":
                ...
                numEliminated++;
                if (isPlayer) {
                    createPopup("Bad luck! You lose!");
                    GoBetween.hasWon = false;
                    SceneManager.LoadScene("GameEnd");
                } else if (numEliminated >= 3) {
                    createPopup("Congratulations! You win!");
                    GoBetween.hasWon = true;
                    SceneManager.LoadScene("GameEnd");
                } else {
        ...
        for(int i = 0; i < script.grid.getMaxPlayers(); i++)
        {
            if(playerIndex != i)
            {
                ...
                if(currPiece.currY == otherPiece.currY && currPiece.currX == otherPiece.currX)
                {
                    ...
                    numEliminated++;
                    // check if all other pieces have been eliminated
                    if (numEliminated >= 3) {
                        createPopup("You win! Congratulations!");
                        GoBetween.hasWon = true;
                        SceneManager.LoadScene("GameEnd");
                    } else {
    ...
    private void createPopup(string message)
    {
        ...
        textParam.transform.localScale = new Vector3(1.0f, 1.0f, 1);
